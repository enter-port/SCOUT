...
# ---- prerequisites ------------------------------------------------------- #
for f in configs/eval_${TASK}_e2.yaml configs/vib_${TASK}_image_e2.yaml \
...
if [ "$A" = SCOUT ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (SCOUT-round0, vib_${TASK}_image_e2.yaml)"
    RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.train_vib \
      --config configs/vib_${TASK}_image_e2.yaml \
...
with open(f"configs/vib_{task}_image_e2.yaml") as f:
...
cfg["wandb"]["project"] = f"{task.upper()}-experiment2"
