...
| 命中 | 引导动作编码回去 `μ(s,a) ≈ z` | 一致性误差降 |
| 方向对 | Cost 随去噪步降(若升 → `guidance_scale` 取负) | cost 递减 |
| on-manifold | 动作可执行、jerk 低、偏离 DP 默认适中 | — |

**生死诊断(先于一切)**:量 `‖∂μ/∂a‖`。

- guidance 梯度 `∇_a‖z − μ(s,a)‖` 完全靠 μ 对 a 的敏感度;而训练目标里的 `−β·I(Z;A)` 恰恰在压制它。
- β 太大 → μ 不随 a 变 → guidance 是 no-op → 探索死(无论 `guidance_scale` 多大)。
- 做法:对单个 ckpt 算平均 `‖∂μ/∂a‖`,看「a 抖一个 std 时 μ 的相对移动」(敏感比);≪ 1 ⇒ 这版 β 太大,降 β 重训。
...
| 层 | 代码 | 阶段 | 过了说明 |
|---|---|---|---|
| 可行性 | action 级四判据 + `‖∂μ/∂a‖` 生死诊断 | 阶段 1 | guidance 机制成立 |
