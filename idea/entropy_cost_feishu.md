...
$$C(\hat x_0) \;=\; -\min\Big(\mathrm{KL}\big(q_\phi(z\mid\bar s,\,a)\;\big\Vert \;q_\phi(z\mid\bar s,\,a^0)\big),\;\kappa\Big),\qquad a=\mathrm{bridge}(\hat x_0) \tag{2}$$
...
$$\mathrm{KL}(q_a\Vert q_{a^0}) = \frac{1}{2}\sum_{i=1}^{16}\Big[ \frac{(\mu_i-\mu^0_i)^2}{\sigma^{0\,2}_i} + \frac{\sigma^2_i}{\sigma^{0\,2}_i} - 1 - (\mathrm{logvar}_i - \mathrm{logvar}^0_i) \Big] \tag{3}$$
...
$$\min_\phi\; \mathbb{E}\Big[\big\Vert D_\psi(z,\bar s_t)-E_s(o_{t+1})\big\Vert ^2\Big] + \beta\,\mathrm{KL}\big(q_\phi(z\mid\bar s_t,a_t)\,\big\Vert \,\mathcal{N}(0,I)\big),\qquad z=\mu_\phi+\sigma_\phi\odot\epsilon,\ \epsilon\sim\mathcal{N}(0,I) \tag{4}$$
...
$$I(Z;X)\;\ge\;\mathbb{E}_x\,\mathbb{E}_{z\sim q_\phi(\cdot\mid x)}\Big[\log q_\phi(z\mid x)-\log p(z)\Big] \tag{8}$$
...
- 单个样本的判别奖励 $r(a,z):=\log q_\phi(z\mid\bar s,a)-\log p(z)$ 的期望 $\mathbb{E}_{z\sim q_a}[\,\cdot\,]=\mathrm{KL}(q_a\Vert p)$：该动作的后验离先验多远（nats）。
...
$$\mathbb{E}_{z\sim q_a}[\Delta r(z)]=\mathrm{KL}\big(q_a\,\Vert \,q_{a^0}\big) \tag{10}$$

这正是式 (2) 中的 KL。
**与"两个 KL 的差"的关系**：把式 (10) 的比值拆向先验得 $\mathrm{KL}(q_a\Vert q_{a^0})=\mathrm{KL}(q_a\Vert p)-\mathbb{E}_{z\sim q_a}[\log\frac{q_{a^0}}{p}]$，当两后验接近时第二项 ≈ $\mathrm{KL}(q_{a^0}\Vert p)$，故 $\mathrm{KL}(q_a\Vert q_{a^0})\approx\mathrm{KL}(q_a\Vert p)-\mathrm{KL}(q_{a^0}\Vert p)$（候选的绝对不典型度减意图的绝对不典型度）【近似，测度交换，未采用】。
实现直接算式 (10)：它是恒等式，且式 (3) 的马氏加权梯度性质更好。
...
| **SOE**（arXiv:2509.19292） | 损失 $\mathcal{L}_{IL}+\mathcal{L}_{IB}$：模仿项 + 信息瓶颈项（$-\log q_\phi(a\mid z)$ 的扩散重建 + $\beta\,\mathrm{KL}[p_\theta(z\mid o)\Vert r(z)]$，后验只条件于观测）；测试期在潜空间加噪探索 | 对照点：SOE 的 KL 是**后验到先验**（训练正则）；本文的 KL 是**后验到意图后验**（推理探索信号）。SOE 扰动 $z$ 本身，本文经 $\partial q_\phi/\partial a$ 在动作空间引导 |
