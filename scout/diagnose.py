...
def sensitivity_ratio(model, obs_t, A_t, sigma_a, sigma_mu):
    ...
    Exact formula: with ``g = ∂(Σ_k μ_k)/∂A_t`` (one backward pass on
    ``mu.sum()``), ``g[b]`` is the *row-sum* of the per-sample Jacobian
