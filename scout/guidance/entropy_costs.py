...
class AtypicalCostPlanner(ScoutPlanner):
    ...
    def _encode_and_row_losses(self, x0_hat: torch.Tensor,
        ...
        (mu [grad-carrying], per-row capped-KL losses list). Vectorized
        (2026-09-01, 方案二): the per-row python loop cost ~6 kernel
        launches x B per call on the guided hot path; the batched form via
        :func:`_kl_rows` is math-identical (elementwise per element; the
        ``style_dim`` last-dim reduction replaces B per-row ``.sum()``
        calls), and ``unbind`` keeps the historical 0-dim-tensor-per-row
        list contract. Clamp/cap semantics unchanged: the row VALUE caps at
        ``-cap``, the row GRADIENT is masked to zero at/above cap (torch
        clamp(max) backward)."""
        s_bar_t = self._resolve_s_bar_t(current_obs)
        a = _enc_forward(self, x0_hat)
        mu, logvar = self.scout_vib.vib_enc(s_bar_t.detach(), a)
