#!/bin/bash
# cfk2_launch.sh -- COFFEE-MG-p2 one-shot orchestrator (2026-09-22).
# 2 arms (DP baseline + ATY with per-round N1 activity-gate recalibration)
...
for SEED in 233 2333 23333; do
  for ARM in DP ATY; do
    tmux new-session -d -s cfk2_${ARM,,}_s$SEED \
      "cd $ROOT && SEED=$SEED GPU=$g ARM=$ARM NROUNDS=6 bash scripts/coffee/cfk2_chain.sh"
    echo "[cfk2] spawned cfk2_${ARM,,}_s$SEED gpu=$g"
    g=$((g+1))
  done
done
echo "[cfk2] LAUNCH_DONE $(date '+%F %T')"
