#!/bin/bash
# round_cfk3.sh (2026-09-22) -- COFFEE-MG-p3 round driver. COPY of
# scripts/coffee/round_cfk.sh with these deltas ONLY:
#   * WPROJ default COFFEE-MG-p3-s$TSEED;
#   * ATY arm: PER-ROUND n3 multiplicative recalibration (user-specified
#     2026-09-22, replaces the p2 N1 gate; cfk_param_calib.py n3) injected
#     after the walk-back VIB ckpt resolution and BEFORE GEXTRA
#     construction: frozen core batch (torch.manual_seed(0),
#     num_workers=0) + fixed crossed-pair permutation (seed 7) through
#     base dyn + this round's flight dyn give
#       rho = farKL_round/farKL_base   -> ATT_CAP  = kappa0 * rho  (2.5)
#       phi = |g|_round/|g|_base       -> ATY_SCALE = eta0   * phi  (5.6)
#     (|g| = mean elementwise |grad_a KL(q(z|s,a_pi(i)) || q(z|s,a_i))|,
#     query side, reference posterior detached -- same structure as the
#     rollout-time guidance gradient). Pure multiplication: no gate, no
#     threshold, no clamp, no eta=0 branch. Calib json + stdout land in
#     $RDIR/calib_n3_r$NUM.json / calib.stdout (RDIR-level: survives the
#     phase-A restart rm of $RDIR/log). Calib failure = round FATAL.
#   * DRY_RUN skips the calib call (planned dose = ETA0 / ATT_CAP).
...
#
# Dose knobs are DEFAULTS ONLY -- for ATY the per-round n3 calib OVERRIDES
# BOTH ATY_SCALE (eta) AND ATT_CAP (kappa); DP arm is guide=off.
#
# Usage:  round_cfk3.sh <coffee> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 ATY_SCALE=5.6 (fallbacks only)
#         ETA0=5.6 (n3 anchor; kappa0 anchor = ATT_CAP default 2.5)
...
TASK=${1:?usage: round_cfk3.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_cfk3.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_cfk3.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
ATT_CAP=${ATT_CAP:-2.5}         # kappa0 anchor -- n3 calib OVERRIDES per round (kappa0*rho)
ATY_SCALE=${ATY_SCALE:-5.6}     # fallback only -- n3 calib OVERRIDES per round (eta0*phi)
ETA0=${ETA0:-5.6}               # n3 anchor eta0 (p1 grid verdict)
...
WPROJ=${WPROJ:-COFFEE-MG-p3-s${TSEED}}
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_cfk3.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_cfk3.sh $TASK BASE 0)"; exit 1; }
...
# ---- [p3 delta] ATY arm: n3 multiplicative recalibration (per round) ------- #
# Runs BEFORE GEXTRA so the recalibrated dose is what phase A/B actually
# carry (the eval phase is unguided regardless; the dose only bites in
# phase B rescue). Artifacts at $RDIR level (calib_n3_r$NUM.json +
# calib.stdout) survive the phase-A restart rm of $RDIR/log.
# Deterministic: frozen core batch, torch.manual_seed(0), fixed
# crossed-pair permutation (seed 7). BOTH knobs are overwritten:
# ATY_SCALE = eta0*phi, ATT_CAP = kappa0*rho.
if [ "$A" = ATY ]; then
  if [ "${DRY_RUN:-0}" = 1 ]; then
    log "[calib-n3] DRY_RUN: planned n3 calib (base=$(newest_vib "$TDYN/dyn-base") round=$VIBCKPT eta0=$ETA0 kappa0=$ATT_CAP); ATY_SCALE/ATT_CAP stay $ATY_SCALE/$ATT_CAP"
  else
    CALIBJ=$RDIR/calib_n3_r${NUM}.json
    env CUDA_VISIBLE_DEVICES=$GPU $PY scripts/coffee/cfk_param_calib.py n3 \
      --seed-data-root "$DATA_ROOT" \
      --base-vib "$(newest_vib "$TDYN/dyn-base")" \
      --round-vib "$VIBCKPT" \
      --dp-ckpt "$DPCKPT" \
      --eta0 "$ETA0" --kappa0 "$ATT_CAP" \
      --out "$CALIBJ" > "$RDIR/calib.stdout" 2>&1
    RC=$?
    [ $RC -ne 0 ] && { log "[calib-n3] FATAL rc=$RC -- see $RDIR/calib.stdout"; exit 1; }
    ...
    DOSEV=$($PY -c "import json;d=json.load(open('$CALIBJ'));print('rho=%.4g phi=%.4g'%(d['rho'],d['phi']))")
    log "[calib-n3] round=$NUM $DOSEV -> ATY_SCALE=$ATY_SCALE ATT_CAP=$ATT_CAP (json: $CALIBJ)"
  fi
fi
