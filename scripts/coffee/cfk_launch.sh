#!/bin/bash
# cfk_launch.sh -- COFFEE-MG-p1 MASTER ORCHESTRATOR (2026-09-20).
# COPY of scripts/coffee_prep/ckp_launch.sh (the flow the user reran on
# coffee: "用一样的流程跑一下coffee"), task/paths/names adapted. Run ONCE
# inside tmux on 1022:
#   A) wait for the pool download (nohup curl may still be running) + verify
#   B) per-seed core20 prep (cfk_prep_data.sh x3: split + abs conversion +
#      reset-reproducibility probe)
#   C) round0 x3 seeds in parallel (tmux cfk_s<seed>_BASE on 1022 GPU 0/1/2;
#      DP-base 600ep b64 + dyn-base) -> wait for the 3 ROUND0 TOTAL lines
#   D) grid on 1022 LOCAL (tmux cfk_grid, GPUs 0-6; driver: grad probe ->
#      preseed eval -> dose probes -> placebo -> coarse ATY -> adaptive loop
#      -> ORBIT at TOP-2 ATY etas -> SUM-criterion verdict.json)
...
#      baojiachun; shared CPFS verified 2026-09-20 via marker probe). One
#      chain per GPU.
# usage: tmux new-session -d -s cfk_launch 'bash scripts/coffee/cfk_launch.sh'
set -uo pipefail
...
cd "$ROOT" || exit 1
mkdir -p "$CAMP" "$GRIDROOT"   # stage A/B append logs under $CAMP -- dir must exist
...
log "A: waiting for pool download (1.86G from hf-mirror) ..."
T0=$(date +%s)
while true; do
  SZ=$(stat -c%s "$POOL" 2>/dev/null || echo 0)
  [ "$SZ" -ge "$EXPECT_BYTES" ] && { log "A: pool complete ($SZ bytes)"; break; }
  if ! pgrep -f "curl.*coffee_d0" > /dev/null 2>&1; then
    log "A: no active download and size $SZ < $EXPECT_BYTES -- (re)starting download"
    bash scripts/coffee/cfk_download_pool.sh >> "$CAMP/pool_download.log" 2>&1 \
...
for s in $SEEDS; do
  log "B: prep s$s ..."
  bash scripts/coffee/cfk_prep_data.sh "$s" || { log "FATAL: prep s$s failed"; exit 1; }
done

# ---------------- C) round0 x3 (parallel on 1022) + wait --------------------- #
for spec in "233:0" "2333:1" "23333:2"; do
  s=${spec%%:*}; g=${spec##*:}
  RL=$CAMP/COFFEE-MG-p1-s$s/coffee/round.log
  grep -q "ROUND0 coffee seed=$s TOTAL" "$RL" 2>/dev/null && { log "C: round0 s$s already done -- skip"; continue; }
  if tmux has-session -t "cfk_s${s}_BASE" 2>/dev/null; then
    log "C: tmux cfk_s${s}_BASE exists -- reuse"
  else
    tmux new-session -d -s "cfk_s${s}_BASE" \
      "cd $ROOT && SEED=$s GPU=$g ARM=BASE NROUNDS=6 bash scripts/coffee/cfk_chain.sh"
    log "C: round0 s$s launched @1022 GPU$g (tmux cfk_s${s}_BASE)"
  fi
done
T0=$(date +%s)
while true; do
  done_n=0
  for s in $SEEDS; do
    grep -q "ROUND0 coffee seed=$s TOTAL" "$CAMP/COFFEE-MG-p1-s$s/coffee/round.log" 2>/dev/null && done_n=$((done_n+1))
  done
...
for s in $SEEDS; do tmux kill-session -t "cfk_s${s}_BASE" 2>/dev/null || true; done

# ---------------- D) grid on 1022 LOCAL -------------------------------------- #
if [ -f "$GRIDROOT/verdict.json" ] && ! grep -q '"error"' "$GRIDROOT/verdict.json" 2>/dev/null; then
  log "D: grid verdict already present -- skip"
else
  mkdir -p "$GRIDROOT"
  if tmux has-session -t cfk_grid 2>/dev/null; then
    log "D: tmux cfk_grid already exists (1022 local) -- reuse"
  else
    tmux new-session -d -s cfk_grid "GPUS=\"0 1 2 3 4 5 6\" bash scripts/coffee/cfk_grid_driver.sh >> $GRIDROOT/driver_console.log 2>&1" \
      || { log "FATAL: failed to spawn cfk_grid locally"; exit 1; }
    log "D: grid driver spawned on 1022 (tmux cfk_grid, GPUs 0-6)"
  fi
  T0=$(date +%s)
  while true; do
    if [ -f "$GRIDROOT/verdict.json" ] && ! grep -q '"error"' "$GRIDROOT/verdict.json" 2>/dev/null; then
    ...
    if ! tmux has-session -t cfk_grid 2>/dev/null; then
      log "FATAL: cfk_grid tmux gone and no verdict (see $GRIDROOT/driver_console.log)"; exit 1
    fi
...
ATY_SCALE=$ATY_SCALE ATT_CAP=$ATT_CAP ORB_SIGMA=$ORB_SIGMA ORB_LAM=$ORB_LAM \
  bash scripts/coffee/cfk_start_arms.sh 233  0 1 2 \
  || { log "FATAL: start_arms s233 failed"; exit 1; }
ATY_SCALE=$ATY_SCALE ATT_CAP=$ATT_CAP ORB_SIGMA=$ORB_SIGMA ORB_LAM=$ORB_LAM \
  bash scripts/coffee/cfk_start_arms.sh 2333 3 4 5 \
  || { log "FATAL: start_arms s2333 failed"; exit 1; }
log "F: starting arms s23333 (1024 GPU0/1/2) via ssh"
$SSH4 "cd $ROOT && ATY_SCALE=$ATY_SCALE ATT_CAP=$ATT_CAP ORB_SIGMA=$ORB_SIGMA ORB_LAM=$ORB_LAM bash scripts/coffee/cfk_start_arms.sh 23333 0 1 2" \
