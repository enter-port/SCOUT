#!/bin/bash
# round_cfk2.sh (2026-09-22) -- COFFEE-MG-p2 round driver. COPY of
# scripts/coffee/round_cfk.sh with these deltas ONLY:
#   * WPROJ default COFFEE-MG-p2-s$TSEED;
#   * ATY arm: PER-ROUND N1 activity-gate recalibration (param-dev
#     2026-09-21/22, cfk_param_calib.py n1) injected after the walk-back
#     VIB ckpt resolution and BEFORE GEXTRA construction: one no-grad
#     forward of the frozen core batch through base dyn + this round's
#     flight dyn gives the far-field (crossed-pair) KL retention
#     rho = farKL_round/farKL_base; rho < RHO_MIN (default 0.5) ->
#     ATY_SCALE=0 (gate open: guidance channel degraded, fall back to the
#     DP prior's own retries); else keep the calibrated dose ETA0 (5.6,
#     p1 grid verdict). ATT_CAP stays 2.5 (kappa semantics = dimensionless
#     tilt bound, scale-free). Calib json + stdout land in
#     $RDIR/calib_n1_r$NUM.json / calib.stdout (RDIR-level: survives the
#     phase-A restart rm of $RDIR/log). Calib failure = round FATAL.
#   * DRY_RUN skips the calib call (planned dose = ETA0).
# Everything else inherited VERBATIM from round_cfk.sh (2026-09-20): see
# its header for the full lineage (three arms, eval-pk, two-phase sharded
# rescue, TSEED determinism, DP 300ep b256 / dyn 100ep SOE budget,
# walk-back, anti-deadlock retrain, heartbeat, wandb backfill).
#
# Dose knobs are DEFAULTS ONLY -- for ATY the per-round calib OVERRIDES
# ATY_SCALE; DP arm is guide=off.
#
# Usage:  round_cfk2.sh <coffee> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 ATY_SCALE=<fallback only>
#         ETA0=5.6 RHO_MIN=0.5 (N1 gate)
#         ORB_* (unused by DP/ATY) DYN_FREEZE_AFTER=6 CORE_N=20 DP_BS=256
#         SHARD_P=5 SHARD_ENVS=25 EVALNENV=25 ETRIES=5 FLUSH_EVERY=100
# Layout: $DATA_ROOT/coffee/{rollout/,train/DP/,train/dyn/}.
set -u

TASK=${1:?usage: round_cfk2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_cfk2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_cfk2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
ATT_CAP=${ATT_CAP:-2.5}         # entropy cost: KL-bonus cap kappa (calibrated; N1 keeps it fixed)
ATY_SCALE=${ATY_SCALE:-5.6}     # fallback only -- ATY arm OVERRIDES via the n1 calib below
ETA0=${ETA0:-5.6}               # N1 gate: dose kept when rho >= RHO_MIN (p1 grid verdict)
RHO_MIN=${RHO_MIN:-0.5}         # N1 gate: far-field retention threshold
...
WPROJ=${WPROJ:-COFFEE-MG-p2-s${TSEED}}
...
for f in configs/eval_${TASK}_entropy.yaml configs/vib_${TASK}_exp1.yaml \
         ...
         scripts/coffee/cfk_param_calib.py; do
  [ -f "$f" ] || { echo "missing $f"; exit 1; }
done
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_cfk2.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_cfk2.sh $TASK BASE 0)"; exit 1; }
...
# ---- [p2 delta] ATY arm: N1 activity-gate recalibration (per round) ------- #
# Runs BEFORE GEXTRA so the gated dose is what phase A/B actually carry (the
# eval phase is unguided regardless; the dose only bites in phase B rescue).
# Artifacts at $RDIR level (calib_n1_r$NUM.json + calib.stdout) survive the
# phase-A restart rm of $RDIR/log. Deterministic: frozen core batch,
# torch.manual_seed(0), fixed crossed-pair permutation (seed 7).
if [ "$A" = ATY ]; then
  if [ "${DRY_RUN:-0}" = 1 ]; then
    log "[calib-n1] DRY_RUN: planned n1 calib (base=$(newest_vib "$TDYN/dyn-base") round=$VIBCKPT eta0=$ETA0 rho_min=$RHO_MIN); ATY_SCALE stays $ATY_SCALE"
  else
    CALIBJ=$RDIR/calib_n1_r${NUM}.json
    env CUDA_VISIBLE_DEVICES=$GPU $PY scripts/coffee/cfk_param_calib.py n1 \
      --seed-data-root "$DATA_ROOT" \
      --base-vib "$(newest_vib "$TDYN/dyn-base")" \
      --round-vib "$VIBCKPT" \
      --dp-ckpt "$DPCKPT" \
      --eta0 "$ETA0" --kappa0 "$ATT_CAP" --rho-min "$RHO_MIN" \
      --out "$CALIBJ" > "$RDIR/calib.stdout" 2>&1
    RC=$?
    [ $RC -ne 0 ] && { log "[calib-n1] FATAL rc=$RC -- see $RDIR/calib.stdout"; exit 1; }
    ATY_SCALE=$($PY -c "import json;print('%.6g'%json.load(open('$CALIBJ'))['eta'])")
    RHOV=$($PY -c "import json;d=json.load(open('$CALIBJ'));print('%.4g gate_open=%s'%(d['rho'],d['gate_open']))")
    log "[calib-n1] round=$NUM rho/gate: $RHOV -> ATY_SCALE=$ATY_SCALE kappa=$ATT_CAP (json: $CALIBJ)"
  fi
fi

T0=$(date +%s)
DOSELOG=""
[ "$A" = ATY ] && DOSELOG=" aty_scale=$ATY_SCALE"
...
if [ "${SKIP_ROLLOUT:-0}" = 1 ] && [ -f "$RDIR/all.hdf5" ]; then
  ...
  log "[1/3b] explore phase: $SHARD_P workers x n_envs=$SHARD_ENVS guide=$GUIDE scale=$ATY_SCALE failed-of-eval(x$ETRIES) flush_every=$FLUSH_EVERY -> merged $EXPLORE_JSON"
  HB_PID=""
  if [ "${DRY_RUN:-0}" != 1 ]; then
