#!/bin/bash
# round_cfk.sh (2026-09-20) -- COFFEE-MG-p1 round driver (MimicGen coffee_d0
# campaign). COPY of scripts/coffee_prep/round_ckp.sh with these deltas ONLY:
#   * task = coffee (MimicGen, env_name Coffee_D0, horizon 400;
#     configs/vib_coffee_exp1.yaml + configs/eval_coffee_entropy.yaml
#     + configs/base_dp_coffee_image.yaml, all new 2026-09-20);
#   * round0 OFFICIAL = the MimicGen coffee_d0 pool (400 mg-generated demos,
#     obs pre-rendered 84x84, states+actions OSC_POSE). The split's 20-demo
#     core has NO abs_actions out of split_core.py -- the conversion to 7-dim
#     absolute actions is done ONCE per seed by scripts/coffee/cfk_prep_data.sh
#     BEFORE round0 (robomimic dataset_conversion replays states in
#     Coffee_D0), so round0 finds $CORE already converted and skips the split
#     (same "core exists -- skip" branch as threading's copied-base flow);
#   * WPROJ default COFFEE-MG-p1-s$TSEED.
# Everything else inherited verbatim from round_ckp.sh/round_threading.sh:
# PY = /root/workspace/baojiachun/.venv_mg/bin/python (robomimic's
# env_robosuite imports mimicgen so Coffee_D0 is registered for EVERY env
# creation); THREE arms (DP: guide off / ATY: --guide atypical raw dose /
# ORBIT: --guide orbit same raw dose + lam/delta/sigma x0.5^(r-1)/anneal2/
# fb-soft, NO dimless), ETRIES=5 (pass@5), SHARD_P=5/arm x25env, MODE=full|
# eval-only|eval-pk, two-phase sharded rescue, TSEED determinism, DP 300ep
# b256 retrain (BASE DP stays b64) / dyn 100ep SOE budget, walk-back,
# anti-deadlock retrain, heartbeat reporter, wandb backfill, flush-every.
#
# Dose knobs are DEFAULTS ONLY -- cfk_start_arms.sh pins the final post-grid
# values explicitly per arm (user flow: round0 -> grid on s233 -> arms start).
#
# Usage:  round_cfk.sh <coffee> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
...
TASK=${1:?usage: round_cfk.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_cfk.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_cfk.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
ATY_SCALE=${ATY_SCALE:-0.5}     # atypical/orbit RAW dose (coffee dose set by the s233 grid; NO dimless)
...
WPROJ=${WPROJ:-COFFEE-MG-p1-s${TSEED}}
...
if [ "$A" = BASE ]; then
  ...
  if [ -f "$CORE" ]; then
    ...
    # loader error. The canonical path is cfk_prep_data.sh (split + abs
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_cfk.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_cfk.sh $TASK BASE 0)"; exit 1; }
