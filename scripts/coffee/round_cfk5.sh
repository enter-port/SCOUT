#!/bin/bash
# round_cfk5.sh (2026-09-23) -- COFFEE-MG-p5 round driver. COPY of
# scripts/coffee/round_cfk4.sh with the THM3 recipe ported (user order
# 2026-09-23: "重新开始跑coffee-p5,要求和threading中一样,包括keppa重标定,
# 同时每一轮的训练轮次和batch size都和threading完全一样"):
#   * WPROJ default COFFEE-MG-p5-s$TSEED;
#   * ATY arm: per-round R-ANCHORED eta re-calibration (inherited verbatim
#     from p4, cfk_rcalib.py) FOLLOWED by per-round kappa v2 C-anchored
#     recalibration (verbatim THM3 method, scripts/threading/thm2_c_calib.py,
#     commit a6d004d): C = step-averaged UNCAPPED KL on the frozen core
#     batch, kappa' = kappa * C_target / C_mean (inverted ratio), band
#     +-10% of C_target, at most 2 repeats, C_target measured on the BASE
#     pair at BASE_ETA (per-seed constant; cfk5_launch.sh measures it once
#     after the beta=1e-5 dyn-base retrain and stores it in
#     $DATA_ROOT/coffee/rollout/base_rcalib.json) and fixed base_kappa=2.5.
#     The round's initial kappa0 = previous
#     round's kappa (calib_kcal json -> calib_rcalib json -> ATT_CAP
#     fallback). Calib jsons: calib_rcalib_r$NUM.json (eta) +
#     calib_kcal_r$NUM.json (kappa) at $RDIR level. EITHER calib failure
#     = round FATAL. Runs BEFORE GEXTRA so the calibrated (eta, kappa) is
#     what phase B actually carries (eval phase unguided regardless).
#   * DP retrain budget = the B2 recipe (THM3-identical): DP_EPOCHS_SOE
#     default 600, DP_BS default 64, ckpt_every 300 -> ckpts 299/599;
#   * dyn retrain budget = the A3 recipe (THM3-identical):
#     DYN_EPOCHS_SOE default 300, BETA=1.0e-5 written into EVERY dyn
#     training config (round0 dyn-base included; cfk5_launch.sh retrains
#     dyn-base at beta=1e-5 instead of copying p1's beta=3e-5 one);
#   * explore phase B runs with --stop-on-first-success (THM3 recording:
#     an init's first success closes it -- ONE successful traj per solved
#     scene; pass@k unaffected);
#   * SHARD_P stays 5 (coffee-family EGL hard clamp, 2026-09-20 grid:
#     8x25 AND 8x20 both OOM a 98G H20) -- NOT raised to threading's 8.
# Everything else inherited VERBATIM from round_cfk4.sh (three arms,
# eval-pk final round, two-phase sharded rescue, TSEED determinism,
# walk-back, anti-deadlock retrain, heartbeat, wandb backfill).
#
# Dose knobs are DEFAULTS ONLY -- for ATY the per-round calib OVERWRITES
# ATY_SCALE (eta) and ATT_CAP (kappa); DP arm is guide=off.
#
# Usage:  round_cfk5.sh <coffee> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 ATY_SCALE=5.6 (fallbacks only)
#         ETA0=5.6 (rcalib eta anchor for round 1; kappa anchor 2.5)
#         BASE_ETA=<float> (kappa v2 C-anchor on the base pair -- per-seed
#         value from cfk5_launch.sh; REQUIRED for ATY arms)
#         BETA=1.0e-5 ORB_* DYN_FREEZE_AFTER=6 CORE_N=20 DP_BS=64
#         DP_EPOCHS_SOE=600 DYN_EPOCHS_SOE=300
#         SHARD_P=5 SHARD_ENVS=25 EVALNENV=25 ETRIES=5 FLUSH_EVERY=100
# Layout: $DATA_ROOT/coffee/{rollout/,train/DP/,train/dyn/}.
...
TASK=${1:?usage: round_cfk5.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_cfk5.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_cfk5.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
  ...
  coffee) TASKUP=COFFEE ;;
  *) echo "task must be can, square, transport, tool_hang, threading, coffee_prep or coffee (got: $TASK)"; exit 1 ;;
esac
case "$A" in
...
ATT_CAP=${ATT_CAP:-2.5}         # kappa anchor (fallback; per-round calib OVERWRITES)
ATY_SCALE=${ATY_SCALE:-5.6}     # fallback only -- rcalib OVERWRITES per round (R->0.01)
ETA0=${ETA0:-5.6}               # rcalib eta anchor for ROUND 1 (p1 grid verdict)
BETA=${BETA:-1.0e-5}            # [p5] VIB beta for EVERY dyn training (THM3 A3 recipe)
...
CORE_N=${CORE_N:-20}            # round0 split: 20 of 1000 mg demos (threading/coffee_prep precedent)
DP_BS=${DP_BS:-64}              # [p5] [2/3] DP retrain batch = B2 recipe (THM3: b64)
B256ARGS=(dataloader.batch_size="$DP_BS" val_dataloader.batch_size="$DP_BS")
SHARD_P=5                   # explore shard workers per arm -- HARD CLAMP: MG coffee-family EGL 0.4-0.6G/env (coffee_prep grid 2026-09-20: 8x25 AND 8x20 both OOM a 98G H20)
...
WPROJ=${WPROJ:-COFFEE-MG-p5-s${TSEED}}
...
# ============================================================================ #
# ROUND 0 (arm BASE): seeded split + base DP + dyn-base (beta=1e-5 retrain)
# ============================================================================ #
if [ "$A" = BASE ]; then
  OFFICIAL=/root/workspace/baojiachun/scout/data/robomimic/coffee/mg/coffee_d0.hdf5
  ...
  if [ -f "$CORE" ]; then
    log "[0/3] core exists -- skip split ($CORE; abs conversion done by cfk_prep_data.sh)"
  else
    log "[0/3] FATAL: $CORE missing -- run scripts/coffee/cfk_prep_data.sh <seed> first (split + abs conversion)"
    exit 1
  fi
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_cfk5.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_cfk5.sh $TASK BASE 0)"; exit 1; }
...
# ---- [p4 inherited] ATY arm: R-anchored eta re-calibration (per round) ----- #
...
# ATY round's calib jsons (eta from rcalib; kappa from kcal preferred, the
# rcalib json only carries it through unchanged). Deterministic: frozen
# core batch, same x_T draw (torch.manual_seed(0)) across all eta
# evaluations. ATY_SCALE is OVERWRITTEN with the converged eta.
if [ "$A" = ATY ]; then
  ETA_PREV=$ETA0
  KAP_PREV=$ATT_CAP
  if [ "$CALIB_MODE" = rc ] && [ "$NUM" -gt 1 ]; then
    ...
    if [ -f "$PREVJ" ]; then
      ...
      # [p5] kappa_prev: prefer the previous round's KAPPA calib json -- the
  ...
  if [ "$CALIB_MODE" = pr ]; then
    ...
    # ---- [p5 delta] kappa v2 C-anchored recalibration (THM3 method) ---- #
    ...
    # kappa * C_target/C_mean (inverted ratio); C_target measured on the
    # BASE pair at BASE_ETA (per-seed constant from cfk5_launch.sh) and
    # fixed base_kappa=2.5;
    # kappa0 = kappa_prev. Non-arrival within max-repeat takes the LAST
    # value (rcalib convention). ATT_CAP is OVERWRITTEN with the result.
    [ -n "${BASE_ETA:-}" ] || { log "[calib-kcal] FATAL BASE_ETA not set (per-seed base anchor from cfk5_launch.sh)"; exit 1; }
...
if [ "${SKIP_ROLLOUT:-0}" = 1 ] && [ -f "$RDIR/all.hdf5" ]; then
  ...
  # [p5] --stop-on-first-success: THM3 recording -- an init's first success
  # closes it (ONE successful traj per solved scene; pass@k unaffected).
  log "[1/3b] explore phase: $SHARD_P workers x n_envs=$SHARD_ENVS guide=$GUIDE scale=$ATY_SCALE cap=$ATT_CAP stop_on_first_success failed-of-eval(x$ETRIES) flush_every=$FLUSH_EVERY -> merged $EXPLORE_JSON"
  HB_PID=""
  if [ "${DRY_RUN:-0}" != 1 ]; then
...
PYEOF
  ...
  # -- wandb backfill: explore/pass@{ETRIES} into the phase-A run
...
# eval-pk (口径定案): final round must carry SR AND pass@K. It already ran
# the FULL two-phase rescue rollout above (phase A = eval first-try SR +
# frozen failed set; phase B = sharded rescue x$ETRIES -> merged json
# pass_at_5 = (bs+rescued)/n), so only the retrains are skipped -- unlike
# eval-only, which rolls 1 try/scene and emits SR alone.
if [ "$MODE" != "full" ]; then
...
if [ "$XMODE" = soe ]; then
  ...
  CKE=300                   # [p5] B2 recipe: 600ep b64 -> ckpts 299/599
...
DYN_EPOCHS=0
[ "$XMODE" = soe ] && DYN_EPOCHS=${DYN_EPOCHS_SOE:-300}   # [p5] A3 recipe
...
cfg["beta"] = float(beta)
if int(dyn_ep) > 0:
    cfg["num_epochs"] = int(dyn_ep)   # soe fixed budget (A3: 300)
cfg["save_dir"] = outdyn
