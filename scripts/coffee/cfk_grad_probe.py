"""Offline KL-cost gradient telemetry for COFFEE-MG-p1 (2026-09-20).

COPY of scripts/coffee_prep/ckp_grad_probe.py with ONLY the task/config
paths changed (coffee_prep -> coffee, campaign root 2026_9_20_coffee_mg_p1);
the math and the ladder are untouched.
...
Run (server):
  cd /root/workspace/baojiachun/scout && CUDA_VISIBLE_DEVICES=0 \
    ...
    scripts/coffee/cfk_grad_probe.py --seed-data-root \
...
def main():
    ...
    ap.add_argument("--seed-data-root", required=True,
                    help="COFFEE-MG-p1-s<seed> dir (uses its dyn-base + DP-base + core)")
    ap.add_argument("--batch-size", type=int, default=128)
    ap.add_argument("--out", default=None)
    args = ap.parse_args()

    sdr = args.seed_data_root
    task_dir = os.path.join(sdr, "coffee")
    core = os.path.join(task_dir, "rollout", "coffee_core.hdf5")
    ...
    vib_cfg = yaml.safe_load(open("configs/vib_coffee_exp1.yaml"))
    ...
    ecfg = load_cfg("configs/eval_coffee_entropy.yaml")
