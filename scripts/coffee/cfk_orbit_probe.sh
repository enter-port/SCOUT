#!/bin/bash
# cfk_orbit_probe.sh -- ONE cell of the ORBIT parameter grid for
# COFFEE-MG-p1 (2026-09-20). COPY of scripts/coffee_prep/ckp_orbit_probe.sh
# with these deltas ONLY:
#   * task/paths coffee (campaign 2026_9_20_coffee_mg_p1);
#   * PRESEED optimization (same as cfk_kgrid_probe.sh: canonical frozen
...
#   * WNAME default includes ETA (2026-09-20: the driver runs ORBIT at the
#     TOP-2 ATY etas for the sum criterion, so sigma/lam alone no longer
#     unique-ifies the wandb name).
# Everything else identical: ARM fixed to ORBIT; ETA/KAP pinned per-cell by
# env (the ATY candidate dose); SIG and LAM are REQUIRED parameters ->
...
#          bash scripts/coffee/cfk_orbit_probe.sh
set -u
ARM=ORBIT
ETA=${ETA:?set ETA}
KAP=${KAP:?set KAP}
...
LAM=${LAM:?set LAM}
GPU=${GPU:?set GPU}
CELL=${CELL:-${ARM}_e${ETA}_s${SIG}_l${LAM}}
SEED=233
ETRIES=5
WNAME_BASE=${WNAME_BASE:-${ARM}_e${ETA}_s${SIG}_l${LAM}}
...
[ -f "$BASE_ROOT/coffee/rollout/coffee_core.hdf5" ] || { echo "[ogrid] FATAL: s233 core missing"; exit 1; }
ls "$BASE_ROOT"/coffee/train/DP/DP-base/checkpoints/*.ckpt >/dev/null 2>&1 || { echo "[ogrid] FATAL: s233 DP-base missing"; exit 1; }
ls "$BASE_ROOT"/coffee/train/dyn/dyn-base/*/scout_vib.ckpt >/dev/null 2>&1 || { echo "[ogrid] FATAL: s233 dyn-base missing"; exit 1; }
