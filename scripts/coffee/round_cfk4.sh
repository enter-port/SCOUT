#!/bin/bash
# round_cfk4.sh (2026-09-22) -- COFFEE-MG-p4 round driver. COPY of
# scripts/coffee/round_cfk3.sh with these deltas ONLY:
#   * WPROJ default COFFEE-MG-p4-s$TSEED;
#   * ATY arm: per-round R-ANCHORED eta re-calibration (user-specified
#     2026-09-22, replaces the p3 n3; cfk_rcalib.py) injected after the
#     walk-back VIB ckpt resolution and BEFORE GEXTRA construction:
#     (1) frozen core batch B=128 states; (2) measure R_mean with the
#     PREVIOUS round's dose (eta_{i-1}, kappa_{i-1}) via the real guided
#     denoise loop (same construction as r_probe_rollout.py);
#     (3) eta_i = eta_{i-1} * 0.01 / R_mean; (4) re-measure R_mean at
#     eta_i, success iff 0.009 <= R_mean <= 0.011, else repeat (2)(3),
#     at most 2 repeats. kappa is CARRIED OVER unchanged (2.5).
#     eta_{i-1} for round 1 = ETA0 (5.6, p1 grid verdict); later rounds
#     read $DATA_ROOT/coffee/rollout/ATY-exp$((NUM-1))/calib_rcalib_r*.json.
#     Calib json + stdout land in $RDIR/calib_rcalib_r$NUM.json /
#     calib.stdout (RDIR-level: survives the phase-A restart rm of
#     $RDIR/log). Calib failure = round FATAL.
...
#
# Dose knobs are DEFAULTS ONLY -- for ATY the per-round rcalib OVERWRITES
# ATY_SCALE (eta); kappa stays at the carried-over ATT_CAP; DP arm is
# guide=off.
#
# Usage:  round_cfk4.sh <coffee> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 ATY_SCALE=5.6 (fallbacks only)
#         ETA0=5.6 (rcalib eta anchor for round 1; kappa carried over)
...
TASK=${1:?usage: round_cfk4.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_cfk4.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_cfk4.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
WPROJ=${WPROJ:-COFFEE-MG-p4-s${TSEED}}
...
# ---- [p4 delta] ATY arm: R-anchored eta re-calibration (per round) -------- #
...
if [ "$A" = ATY ]; then
  ...
  if [ "${DRY_RUN:-0}" = 1 ]; then
    ...
    env CUDA_VISIBLE_DEVICES=$GPU $PY scripts/coffee/cfk_rcalib.py \
      ...
      --dp-ckpt "$DPCKPT" \
      --vib-ckpt "$VIBCKPT" \
      --core-hdf5 "$DATA_ROOT/$TASK/rollout/${TASK}_core.hdf5" \
    ...
    log "[calib-rcalib] round=$NUM $DOSEV -> ATY_SCALE=$ATY_SCALE ATT_CAP=$ATT_CAP (json: $CALIBJ)"
  fi
fi
