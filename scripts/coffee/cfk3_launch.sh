#!/bin/bash
# cfk3_launch.sh -- COFFEE-MG-p3 one-shot orchestrator (2026-09-22).
# 2 arms (DP baseline + ATY with per-round n3 multiplicative recalibration
#   eta_r=eta0*phi, kappa_r=kappa0*rho; user-specified 2026-09-22)
...
for SEED in 233 2333 23333; do
  ...
  S2=$DST/COFFEE-MG-p3-s$SEED/coffee
  DPNEW=$(ls -t $S1/train/DP/DP-base/checkpoints/*.ckpt 2>/dev/null | head -1)
  [ -n "$DPNEW" ] || { echo "[cfk3] FATAL no DP-base ckpt for s$SEED"; exit 1; }
  [ -n "$(ls -t $S1/train/dyn/dyn-base/*/scout_vib.ckpt 2>/dev/null | head -1)" ] \
    || { echo "[cfk3] FATAL no dyn-base ckpt for s$SEED"; exit 1; }
  [ -f "$S1/rollout/coffee_core.hdf5" ] || { echo "[cfk3] FATAL missing source core for s$SEED"; exit 1; }
  ...
  copy_chk(){  # copy with SIZE verification (a half-written leftover re-copies)
    ...
    if [ ! -f "$dstf" ] || [ "$(stat -c%s "$srcf")" != "$(stat -c%s "$dstf")" ]; then
      cp -L "$srcf" "$dstf" || { echo "[cfk3] FATAL copy failed: $srcf"; exit 1; }
    fi
  }
  ...
  # seed the shared round.log so cfk3_chain.sh finds it (chains append)
  [ -f "$S2/round.log" ] || echo "[$(date '+%F %T')] [cfk3] base three-piece copied from COFFEE-MG-p1-s$SEED (core + DP-base $DPNEW + dyn-base; no round0)" >> "$S2/round.log"
  echo "[cfk3] s$SEED base ready: core=$(stat -c%s $S2/rollout/coffee_core.hdf5)B dp=$(ls -t $S2/train/DP/DP-base/checkpoints/*.ckpt | head -1 | xargs basename) vib=$(ls -t $S2/train/dyn/dyn-base/*/scout_vib.ckpt | head -1 | xargs basename)"
done

if [ "${COPY_ONLY:-0}" = 1 ]; then
  echo "[cfk3] COPY_ONLY=1 -- base staged, not spawning"
  exit 0
fi
...
for SEED in 233 2333 23333; do
  for ARM in DP ATY; do
    tmux new-session -d -s cfk3_${ARM,,}_s$SEED \
      "cd $ROOT && SEED=$SEED GPU=$g ARM=$ARM NROUNDS=6 bash scripts/coffee/cfk3_chain.sh"
    echo "[cfk3] spawned cfk3_${ARM,,}_s$SEED gpu=$g"
    g=$((g+1))
  done
done
echo "[cfk3] LAUNCH_DONE $(date '+%F %T')"
