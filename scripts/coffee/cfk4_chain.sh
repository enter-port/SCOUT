#!/bin/bash
# cfk4_chain.sh -- COFFEE-MG-p4 chain wrapper (2026-09-22).
# COPY of scripts/coffee/cfk3_chain.sh with these deltas ONLY:
#   * DATA_ROOT default data/2026_9_22_coffee_mg_p4/COFFEE-MG-p4-s$SEED
#     (fresh campaign; base three-piece PRE-COPIED from p1 by
#     cfk4_launch.sh -- chains start at round 1, no round0);
#   * WPROJ = COFFEE-MG-p4-s$SEED;
#   * round script = scripts/coffee/round_cfk4.sh (ATY arm carries the
#     per-round R-anchored eta re-calibration; DP arm guide=off);
#   * exports ETA0=5.6 (rcalib eta anchor for round 1; kappa carried over
#     at 2.5). No RHO_MIN -- rcalib has no gate.
...
#          bash scripts/coffee/cfk4_chain.sh
set -uo pipefail
...
DATA_ROOT=${DATA_ROOT:-$ROOT/data/2026_9_22_coffee_mg_p4/COFFEE-MG-p4-s$SEED}
WPROJ=COFFEE-MG-p4-s$SEED
...
[ -f "$RL" ] || { echo "[chain] FATAL: no round.log -- base not pre-copied? run cfk4_launch.sh"; exit 1; }
...
for N in $(seq 1 $((NROUNDS-1))); do
  ...
  GPU=$GPU TSEED=$SEED DATA_ROOT=$DATA_ROOT WPROJ=$WPROJ WNAME_BASE=$ARM CORE_N=20 \
    bash scripts/coffee/round_cfk4.sh coffee "$ARM" "$N" full
  rc=$?
...
# xETRIES -> pass@ETRIES in merged json), retrains skipped inside round_cfk4.sh.
echo "[chain] round $NROUNDS (eval-pk) START $(date '+%F %T')"
GPU=$GPU TSEED=$SEED DATA_ROOT=$DATA_ROOT WPROJ=$WPROJ WNAME_BASE=$ARM CORE_N=20 \
  bash scripts/coffee/round_cfk4.sh coffee "$ARM" "$NROUNDS" eval-pk
rc=$?
