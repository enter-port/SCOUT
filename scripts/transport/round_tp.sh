#!/bin/bash
# round_tp.sh (2026-09-13) -- TRANSPORT-9-13-orbit round driver, running in the
# MAIN scout repo (post 2026-09-12 consolidation; old scout-th95 worktree is
# DELETED). COPY of scripts/toolhang/round_th95.sh with these deltas:
#   * REPO = /root/workspace/baojiachun/scout (main checkout, shared CPFS --
#     visible from BOTH port-1022 and port-1024 containers);
#   * task = transport (TASKUP mapping + configs already transport-aware:
#     configs/vib_transport_exp1.yaml + configs/eval_transport_entropy.yaml +
#     configs/base_dp_transport_image.yaml, all new/adapted 2026-09-13);
#   * CORE_N=20 (user order: 20 random demos per seed from official 200) --
#     th95 split 20 demos, NOT tool_hang's 40;
#   * WPROJ default TRANSPORT-9-13-orbit-s$TSEED;
#   * DP_BS=256 injected into [2/3] DP retrain ONLY (dataloader +
#     val_dataloader batch, 2026-09-09 user order "b256+LR 不动, 仅 DP 重训,
#     新链"); round0 BASE DP stays b64 (th95-family protocol).
# Everything else inherited verbatim from round_th95.sh: THREE arms
# (DP: guide off / ATY: --guide atypical raw s0.5+kappa2.5 / ORBIT: --guide
# orbit same raw dose + lam.5/delta.25/sigma.05 x0.5^(r-1)/anneal2/fb-soft),
# ETRIES=5 (pass@5), SHARD_P=8/arm x25env, MODE=full|eval-only|eval-pk
# (final round eval-pk = SR + pass@ETRIES, retrains skipped), two-phase
# sharded rescue, TSEED determinism, DP 300ep / dyn 100ep SOE budget,
# walk-back, anti-deadlock retrain, heartbeat reporter, wandb backfill,
# flush-every.
#
# Dose knobs are DEFAULTS ONLY -- tp13_start_arms.sh pins the final
# post-probe values explicitly per arm (user 2026-09-13 flow: round0 ->
# dose probe -> user picks -> arms start).
#
# Usage:  round_tp.sh <transport> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
...
# Layout: $DATA_ROOT/transport/{rollout/,train/DP/,train/dyn/}.
set -u

TASK=${1:?usage: round_tp.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_tp.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_tp.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
ATY_SCALE=${ATY_SCALE:-0.5}     # atypical/orbit RAW dose (th95 default; transport dose set post-probe; NO dimless)
...
CORE_N=${CORE_N:-20}            # round0 split: 20 of 200 official demos (user 2026-09-13)
...
WPROJ=${WPROJ:-TRANSPORT-9-13-orbit-s${TSEED}}
...
if [ "$A" = BASE ]; then
  ...
  if [ -f "$CORE" ]; then
    ...
    log "[0/3] split: $CORE_N of 200 demos, rng seed $TSEED -> $CORE"
    RUN "$PY" scripts/analysis/split_core.py "$OFFICIAL" "$CORE" "$CORE_N" "$TSEED" \
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_tp.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_tp.sh $TASK BASE 0)"; exit 1; }
