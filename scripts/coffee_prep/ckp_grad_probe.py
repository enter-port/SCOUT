"""Offline KL-cost gradient telemetry for COFFEEPREP-MG-p1 (2026-09-20).

COPY of scripts/threading/thread_grad_probe.py with ONLY the task/config
paths changed (threading -> coffee_prep, campaign root
2026_9_19_coffeeprep_mg_p1); the math and the ladder are untouched.

Why run it before the grid: threading's first eta grid was DOSE-INVARIANT
(rescued 16->16 over eta 0.1..2.0) because the near-anchor KL gradients are
tiny -- the response region only showed up around eta 4-16. This probe
separates "cap clipped everything" from "gradient scale tiny" offline, no
env needed, and yields eta_for_0.1sd / eta_for_0.5sd per delta row: the
coffee grid driver uses their median over the useful delta band as the
coarse eta0 anchor.
...
Run (server):
  cd /root/workspace/baojiachun/scout && CUDA_VISIBLE_DEVICES=0 \
    ...
    scripts/coffee_prep/ckp_grad_probe.py --seed-data-root \
    data/2026_9_19_coffeeprep_mg_p1/COFFEEPREP-MG-p1-s233
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--seed-data-root", required=True,
                    help="COFFEEPREP-MG-p1-s<seed> dir (uses its dyn-base + DP-base + core)")
    ap.add_argument("--batch-size", type=int, default=128)
    ap.add_argument("--out", default=None)
    args = ap.parse_args()

    sdr = args.seed_data_root
    task_dir = os.path.join(sdr, "coffee_prep")
    core = os.path.join(task_dir, "rollout", "coffee_prep_core.hdf5")
    ...
    vib_cfg = yaml.safe_load(open("configs/vib_coffee_prep_exp1.yaml"))
    ...
    ecfg = load_cfg("configs/eval_coffee_prep_entropy.yaml")
