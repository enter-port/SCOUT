#!/bin/bash
# ckp_prep_data.sh -- per-seed core20 materialization for COFFEEPREP-MG-p1
# (2026-09-20). COPY of scripts/threading/thread_prep_data.sh with these
# deltas ONLY: task = coffee_prep (MimicGen coffee_preparation_d0 pool,
# env_name Coffee_Prep_D0; paths 2026_9_19_coffeeprep_mg_p1). Everything
# else identical: MimicGen pool (1000 mg-generated demos, obs PRE-RENDERED
# 84x84 agentview_image + robot0_eye_in_hand_image HWC uint8, states,
# actions (T,7) OSC_POSE) ->
...
#      replaying states in Coffee_Prep_D0 (no rendering; no EGL needed)
#   3) rollout/coffee_prep_core.hdf5 in the campaign root + structure verify
...
# usage: bash scripts/coffee_prep/ckp_prep_data.sh <SEED>
set -euo pipefail
SEED=${1:?usage: ckp_prep_data.sh <SEED>}
...
DST=$ROOT/data/2026_9_19_coffeeprep_mg_p1/COFFEEPREP-MG-p1-s$SEED/coffee_prep
CORE=$DST/rollout/coffee_prep_core.hdf5
STAGE=$DST/rollout/.stage_core_noabs_s$SEED.hdf5

[ -f "$POOL" ] || { echo "[prep] FATAL: pool missing: $POOL (run ckp_download_pool.sh)"; exit 1; }
[ -f "$CORE" ] && { echo "[prep] core exists -- skip ($CORE)"; exit 0; }

# CPFS disk-write probe (df untrustworthy, 2026-09-10 lesson)
dd if=/dev/zero of=$ROOT/data/.ckp_prep_probe bs=1M count=512 oflag=direct 2>/dev/null \
  || dd if=/dev/zero of=$ROOT/data/.ckp_prep_probe bs=1M count=512
rm -f $ROOT/data/.ckp_prep_probe
...
echo "[prep] s$SEED step2: abs_actions conversion (state replay in Coffee_Prep_D0)"
...
assert env_args["env_name"] == "CoffeePreparation_D0", env_args["env_name"]
f.close()
