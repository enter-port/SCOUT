#!/bin/bash
# ckp_kgrid_probe.sh -- ONE cell of the eta x kappa 2-D grid for
# COFFEEPREP-MG-p1 (2026-09-20; user order: grid 范围探针后自动定,不停).
# COPY of scripts/threading/thread_kgrid_probe.sh with these deltas ONLY:
...
#     ~40-70min eval per cell; pass@5 comparability IMPROVES (all cells
#     share the bit-identical failure set);
#   * timeout 10800s hard cap (coffee horizon 800 = 2x threading; rc=124 ->
#     TIMEOUT row, no verdict);
#   * csv = GRID_s233/kgrid_results.csv with kappa + jerk columns (the
#     threading schema gains avg_jerk from the explore json).
...
#          bash scripts/coffee_prep/ckp_kgrid_probe.sh
set -u
ARM=${ARM:?set ARM=ATY}
...
[ -f "$BASE_ROOT/coffee_prep/rollout/coffee_prep_core.hdf5" ] || { echo "[kgrid] FATAL: s233 core missing"; exit 1; }
ls "$BASE_ROOT"/coffee_prep/train/DP/DP-base/checkpoints/*.ckpt >/dev/null 2>&1 || { echo "[kgrid] FATAL: s233 DP-base missing"; exit 1; }
ls "$BASE_ROOT"/coffee_prep/train/dyn/dyn-base/*/scout_vib.ckpt >/dev/null 2>&1 || { echo "[kgrid] FATAL: s233 dyn-base missing"; exit 1; }
