#!/bin/bash
# ckp_start_arms.sh -- start the THREE arms of one COFFEEPREP-MG-p1 seed
# (2026-09-20; 3 seeds x 3 arms = 9 chains, ETRIES=5 pass@5, one chain per
# GPU -- THREADING-P3 9-chain zero-ABORT precedent).
#
# Prereq: round0 TOTAL gate line present in
#   data/2026_9_19_coffeeprep_mg_p1/COFFEEPREP-MG-p1-s<SEED>/coffee_prep/round.log
# (written by the BASE branch of ckp_chain.sh after DP+dyn-base train).
#
# usage: ATY_SCALE=<s> [ORB_SIGMA=0.05 ORB_SIGMA_DECAY=.5 ORB_LAM=.5
#          ORB_DELTA=.25 ORB_ANNEAL=2 ATT_CAP=2.5] \
#        bash scripts/coffee_prep/ckp_start_arms.sh <SEED> <DP_GPU> <ATY_GPU> <ORBIT_GPU>
set -u
SEED=${1:?usage: ckp_start_arms.sh <SEED> <DP_GPU> <ATY_GPU> <ORBIT_GPU>}
G_DP=${2:?set DP GPU}; G_ATY=${3:?set ATY GPU}; G_ORB=${4:?set ORBIT GPU}
ATT_CAP=${ATT_CAP:-2.5}
# Single SHARED dose (user 2026-09-20: eta = combined pass@5 argmax on s233;
# ties -> higher min(ATY,ORBIT) pass@5, then LOWER eta; decided by the user to
# be resolved autonomously -- "如果有并列你来选 不要汇报不要停").
...
DR=$ROOT/data/2026_9_19_coffeeprep_mg_p1/COFFEEPREP-MG-p1-s$SEED
cd "$ROOT" || exit 1

# round0 must be complete before arms start
grep -q "ROUND0 coffee_prep seed=$SEED TOTAL" "$DR/coffee_prep/round.log" 2>/dev/null \
  || { echo "[arms] FATAL: round0 TOTAL gate line missing for s$SEED"; exit 1; }

for spec in "ckp_dp_s$SEED:$G_DP:DP" "ckp_aty_s$SEED:$G_ATY:ATY" "ckp_orbit_s$SEED:$G_ORB:ORBIT"; do
