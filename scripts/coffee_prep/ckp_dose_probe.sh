#!/bin/bash
# ckp_dose_probe.sh -- online dose-response probe for COFFEEPREP-MG-p1
# (2026-09-20). COPY of scripts/threading/thread_dose_probe.sh with these
# deltas ONLY: task/paths coffee_prep (campaign 2026_9_19_coffeeprep_mg_p1)
# and the failed-set source = GRID_s233/PRESEED/failed.json (the canonical
# preseeded eval's frozen failure set) instead of ATY-exp1's. Everything else
# identical: ONE guided explore-only rollout on the s233 FROZEN failed set
# (subset via --scene-slice SLOT:SHARDS -- 0:4 = shard 0 of 4, i.e. ~1/4 of
# the failure set), ATY or ORBIT at a given raw eta. Single process (no
# shards) so the [guidance-telemetry] mean_inject lines survive in
# probe.stdout. timeout 4500s (coffee horizon 800 = 2x threading).
# usage: bash scripts/coffee_prep/ckp_dose_probe.sh <GPU> <ATY|ORBIT> <ETA> [TRIES] [SLICE] [KAP]
set -u
...
S=$CAMP/COFFEEPREP-MG-p1-s233
OUT=$CAMP/DOSEPROBE/${ARM}_eta${ETA}_k${KAP}
mkdir -p "$OUT"
cd "$ROOT" || exit 1
DPCKPT=$(ls -t $S/coffee_prep/train/DP/DP-base/checkpoints/*.ckpt | head -1)
VIBC=$(ls -t $S/coffee_prep/train/dyn/dyn-base/*/scout_vib.ckpt | head -1)
CORE=$S/coffee_prep/rollout/coffee_prep_core.hdf5
...
timeout -k 30 4500 env CUDA_VISIBLE_DEVICES=$GPU SCOUT_RENDER_GPU=$GPU MUJOCO_GL=egl \
  ...
  --config configs/eval_coffee_prep_entropy.yaml --task coffee_prep --exp-num 0 \
