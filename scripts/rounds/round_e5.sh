#!/bin/bash
# SCOUT experiment5 round driver -- clone of round_e2.sh (user 2026-08-19).
# The ONLY scientific difference vs e2: NEXPLORE=50 (mid-budget regime between
# e4-can's 10 and e2's 100). Everything else (eval protocol, seeds, accumulation,
# adaptive epochs, wandb layout) is byte-equivalent to e2.
# Also carries e4's eval-only EGL-teardown tolerance (rc=1 with a complete
# round json is treated as success).
#   `round_e5.sh <task> <DP|SCOUT> <round-num> [full|eval-only]`  (env: GPU=<id>)
set -u

TASK=${1:?usage: round_e5.sh <task> <DP|SCOUT> <round-num> [mode]}
A=${2:?usage: round_e5.sh <task> <DP|SCOUT> <round-num> [mode]}
NUM=${3:?usage: round_e5.sh <task> <DP|SCOUT> <round-num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
NEXPLORE=50             # e5: 50 explore scenes per round (the ONLY diff vs e2)
ETRIES=1
...
# ---- prerequisites ------------------------------------------------------- #
for f in configs/eval_${TASK}_e5.yaml configs/vib_${TASK}_image_e5.yaml \
...
if [ "$A" = SCOUT ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (SCOUT-round0, vib_${TASK}_image_e5.yaml)"
    RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.train_vib \
      --config configs/vib_${TASK}_image_e5.yaml \
...
T0=$(date +%s)
log "=== ROUND(e5) $TASK a=$A round=$NUM mode=$MODE START (GPU$GPU; rollout DP=$DPROLL; eval_seed=$SEED explore_seed=$ESEED) ==="
...
RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.eval.run_rollout \
  --config configs/eval_${TASK}_e5.yaml --task "$TASK" --exp-num "$NUM" \
  --base-dp-ckpt "$DPCKPT" \
  --core-hdf5 "$CORE" \
...
if [ $RC -ne 0 ]; then
  ...
  # round json is fully written (observed 2026-08-19 square r6 / can-e4 r6:
  # EGL_NOT_INITIALIZED at eglMakeCurrent, json complete). If the measurement
...
if [ "$MODE" = "eval-only" ]; then
  ...
  T3=$(date +%s)
  log "=== ROUND(e5) $TASK a=$A round=$NUM TOTAL: $(( (T3-T0)/60 ))m$(( (T3-T0)%60 ))s ==="
  exit 0
fi
...
# ---- [3/3] dyn retrain (SCOUT only) on ACCUMULATED data ------------------ #
if [ "$A" != "DP" ]; then
...
with open(f"configs/vib_{task}_image_e5.yaml") as f:
...
cfg["wandb"]["project"] = f"{task.upper()}-experiment5"
