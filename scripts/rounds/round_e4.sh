#!/bin/bash
# SCOUT experiment2 round driver -- SPLIT eval/explore protocol (user 2026-08-17).
# Strictly serial per round: rollout -> retrain DP -> retrain dyn.
#   `round_e2.sh <task> <DP|SCOUT> <round-num>`  (env: GPU=<cuda id>)
#
...
#       project <TASK>-experiment4 (CAN-experiment4 / SQUARE-experiment4),
...
TASK=${1:?usage: round_e2.sh <task> <DP|SCOUT> <round-num>}
...
# ---- prerequisites ------------------------------------------------------- #
for f in configs/eval_${TASK}_e4.yaml configs/vib_${TASK}_image_e4.yaml \
...
if [ "$A" = SCOUT ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (SCOUT-round0, vib_${TASK}_image_e4.yaml)"
    RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.train_vib \
      --config configs/vib_${TASK}_image_e4.yaml \
...
RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.eval.run_rollout \
  --config configs/eval_${TASK}_e4.yaml --task "$TASK" --exp-num "$NUM" \
  --base-dp-ckpt "$DPCKPT" \
  --core-hdf5 "$CORE" \
...
with open(f"configs/vib_{task}_image_e4.yaml") as f:
...
cfg["wandb"]["project"] = f"{task.upper()}-experiment4"
