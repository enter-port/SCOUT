...
# ---- prerequisites ------------------------------------------------------- #
for f in configs/eval_${TASK}_e2s01.yaml configs/vib_${TASK}_image_e2.yaml \
...
if [ "$A" = SCOUT01 ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (SCOUT01-round0, vib_${TASK}_image_e2.yaml)"
...
RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.eval.run_rollout \
  --config configs/eval_${TASK}_e2s01.yaml --task "$TASK" --exp-num "$NUM" \
  --base-dp-ckpt "$DPCKPT" \
  --core-hdf5 "$CORE" \
