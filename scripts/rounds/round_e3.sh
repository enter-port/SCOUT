...
# ---- prerequisites ------------------------------------------------------- #
for f in configs/eval_${TASK}_e3.yaml configs/vib_${TASK}_image_e3.yaml \
...
if [ "$A" = SCOUT ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (SCOUT-round0, vib_${TASK}_image_e3.yaml)"
...
RUN env CUDA_VISIBLE_DEVICES=$GPU $PY -m scout.eval.run_rollout \
  --config configs/eval_${TASK}_e3.yaml --task "$TASK" --exp-num "$NUM" \
  --base-dp-ckpt "$DPCKPT" \
  --core-hdf5 "$CORE" \
