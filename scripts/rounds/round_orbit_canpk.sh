#!/bin/bash
# round_orbit_canpk.sh (2026-09-08) -- CAN-9-2-orbit seed-replicate driver
# (CAN-9-2-orbit-s2333 / CAN-9-2-orbit-s23333; user order 2026-09-08:
# "identical to CAN-9-2-orbit-s233 except the seed"). COPY of the server
# soe_scripts/round_orbit.sh (the exact file the 9-2 chain ran, worktree
# orbit-dev@e110ffc) with ONE functional delta:
#   * new MODE=eval-pk (final round): IDENTICAL two-phase rescue rollout as
#     full (phase A eval+failed-set, phase B sharded explore x$ETRIES, merge,
#     wandb backfill) and then SKIPS the accum rebuilds + [2/3]/[3/3]
#     retrains -- the final round therefore yields SR AND pass@ETRIES from
#     the arm's latest retrained ckpt, per the 2026-09-07 user order
#     "末轮 eval-only = SR + pass@{ETRIES} 两样都要" (th95 had to patch
#     pass@5 in afterwards -- not repeated here; same mechanism as the
#     reviewed scripts/toolhang/round_th95.sh on main).
# Everything else -- including the full-round path -- is byte-identical to
# what CAN-9-2-orbit-s233 ran.
...
#
# Usage:  round_orbit_canpk.sh <can|square> <BASE|SCOUT|DP> <num> [full|eval-only|eval-pk]
