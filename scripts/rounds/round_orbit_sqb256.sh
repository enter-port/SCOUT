#!/bin/bash
# round_orbit_sqb256.sh (2026-09-09) -- SQUARE-9-1-orbit seed-replicate
# driver (SQUARE-9-1-orbit-s2333 / SQUARE-9-1-orbit-s23333; user order
# 2026-09-09: replicate SQUARE-9-1-orbit-s233 at seeds 2333/23333, after
# both can replicate chains finish). COPY of the deployed
# soe_scripts/round_orbit_canpk.sh (itself the CAN-9-2-orbit-s233
# round_orbit.sh + the eval-pk final-round mode) with ONE functional
# delta on top of eval-pk:
#   * DP training batch 256 (user order 2026-09-09 "在训练DP时把batch
#     size改为256"): dataloader.batch_size + val_dataloader.batch_size
#     overridden to $DP_BS (default 256; yaml default 64) at ALL FOUR
#     train.py call sites (round0 base DP 600ep primary + workers=0
#     retry, [2/3] DP retrain 300ep primary + workers=0 retry). The
#     [3/3] dyn/VIB training is NOT touched. LR and every other
#     hyperparameter stay at the yaml values. 09-09 H20 probe measured
#     b256 at ~45% wall savings per retrain round (次线性 scaling).
# The eval-pk final-round mode is kept (2026-09-07 user order: 末轮
# eval-only = SR + pass@{ETRIES} 两样都要), same as the can replicates.
# Square protocol = what SQUARE-9-1-orbit-s233 settled on (worktree
# orbit-dev@e110ffc round_orbit.sh): ETRIES=10, explore 4 workers x 25
# envs (the 09-03 settled square config; the historical chain ran
# r1-r2 at 2 workers before the user raised it to 4 mid-flight),
# EVALNENV=25, DYN_FREEZE_AFTER=6, orbit v3 dose defaults, DP 300ep /
# dyn 100ep (SOE budget), fresh round0 per seed (no prebuilt square
# base exists for seeds 2333/23333).
...
#       (2026-09-02 user order: the RAM gate + global phase-B flock were
#       REMOVED); shard_heartbeat.py relays worker progress/RSS into the
#       phase-A wandb run.
...
# two arms; orbit explore params: kappa 2.5 / lam 0.5 / delta 0.25 unchanged +
# hparam v3 group (sigma 0.16 x 0.5^(round-1), eta_tilde 0.33 dimless, fb-clamp
# soft, noise-anneal 2) -- the merged orbit-hparam-dev fixes (PR #2 + e110ffc);
# DP retrain 300ep / dyn 100ep (SOE budget); wandb one project PER SEED.
#
# Usage:  round_orbit_sqb256.sh <can|square> <BASE|SCOUT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 DYN_FREEZE_AFTER=6
#         SHARD_P=4(square)/2(other) SHARD_ENVS=25 EVALNENV=25 DP_BS=256
...
# orbit hparam group v3 (2026-09-02 rerun order; fixed cross-task set from
# idea/orbit_calibration_values.md; kappa 2.5 / lam 0.5 / delta 0.25 unchanged)
ORB_SIGMA=${ORB_SIGMA:-0.16}             # tangential-noise ceiling (cross-task geometric mean)
ORB_SIGMA_DECAY=${ORB_SIGMA_DECAY:-0.5}  # sigma_eff = ORB_SIGMA * decay**(round-1)
ORB_ANNEAL=${ORB_ANNEAL:-2}              # per-denoise-step noise exponent p
ORB_FB_CLAMP=${ORB_FB_CLAMP:-soft}       # Newton residual clamp (none|soft)
ETA_TILDE=${ETA_TILDE:-0.33}             # dimensionless climb dose (eta-dimless)
SHARD_P=${SHARD_P:-$([ "$TASK" = square ] && echo 4 || echo 2)}  # 09-03 user order: square 4x25 from its next round (memory check passed: trajectory residency is worker-count-invariant; 4+2=6 heavy workers vs the 16-worker/400-env OOM night); 2x25 elsewhere
...
# DP train batch override (user order 2026-09-09: 256; yaml default 64).
# Injected into ALL FOUR train.py call sites (round0 base primary+retry,
# [2/3] retrain primary+retry) -- DP stages only, dyn/VIB untouched.
DP_BS=${DP_BS:-256}
...
GUIDE=off; GEXTRA=()
[ "$A" = SCOUT ] && { GUIDE=orbit; GEXTRA=(--atypical-cap "$ATT_CAP" --orbit-lam 0.5 --orbit-delta 0.25 --orbit-sigma "$ORB_SIGMA" --orbit-sigma-decay "$ORB_SIGMA_DECAY" --orbit-round "$NUM" --orbit-fb-clamp "$ORB_FB_CLAMP" --orbit-noise-anneal "$ORB_ANNEAL" --orbit-eta-dimless --guidance-scale "$ETA_TILDE"); }
...
if [ "${SKIP_ROLLOUT:-0}" = 1 ] && [ -f "$RDIR/all.hdf5" ]; then
  ...
  # full AND eval-pk: the IDENTICAL two-phase rescue rollout (eval-pk only
  # differs AFTER the rollout -- no accum rebuild, no retrains; see the
  # gate below).
...
if [ "$MODE" != "full" ]; then
  log "[2/3]+[3/3] SKIPPED (MODE=$MODE: eval-only = SR measurement only; eval-pk = SR + pass@${ETRIES} rescue measurement; no accum rebuild, no retrains)"
  T3=$(date +%s)
