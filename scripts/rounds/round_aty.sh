# round_aty.sh -- CAN-aty-try1-s233 variant (2026-09-03 user order):
# COPY of round_orbit.sh (the CAN-9-2-orbit-s233 driver) with ONE delta --
# the SCOUT arm runs the orbit PHASE-1 CLIMB ONLY (= the atypical cost):
# lam=0/delta=0/sigma=0, which the run_rollout help documents as
# bit-identical to --guide atypical; the CAN-9-2 phase-1 dose is kept
# verbatim (eta_tilde 0.33 dimless via --orbit-eta-dimless, kappa
# ATT_CAP=2.5, gst 100 from the can eval config). Launch passes
# ETRIES=1 (pass@10 -> pass@1) and SHARD_P=1 (one explore worker);
# everything else verbatim 9-2 (6 rounds, r6 eval-only, 25 envs,
# DP 300ep / dyn 100ep, seed 233). The bare --guide atypical path has
# no eta-dimless switch, hence the degenerate-orbit implementation.
#!/bin/bash
...
GUIDE=off; GEXTRA=()
[ "$A" = SCOUT ] && { GUIDE=orbit; GEXTRA=(--atypical-cap "$ATT_CAP" --orbit-lam 0 --orbit-delta 0 --orbit-sigma 0 --orbit-eta-dimless --guidance-scale "$ETA_TILDE"); }  # aty-try1: lam=delta=sigma=0 => bit-identical to --guide atypical (phase-1 climb only); 9-2 phase-1 dose eta_tilde kept
