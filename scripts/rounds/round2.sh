...
# ALL experiment outputs live under data/experiment2 (user 2026-08-17) --
...
if [ "$A" = SCOUT ]; then
  ...
  if [ -z "$VIBCKPT" ]; then
    log "no VIB ckpt under $VIBDIR -- training dyn-base first (vib_${TASK}_image_e2.yaml)"
    if [ "$DRY_RUN" != 1 ]; then
...
cfg["wandb"]["project"] = f"2-{task}-dyn"
