#!/bin/bash
# round_thm2.sh (2026-09-22) -- THREADING-MG-p2 round driver. COPY of
# scripts/threading/round_threading.sh with these deltas ONLY:
#   * WPROJ default THREADING-MG-p2-s$TSEED;
#   * BETA=<float> (default 1.0e-5) is written into the dyn training config
#     of EVERY dyn training on this chain -- the round0 dyn-base AND each
#     per-round dyn retrain (user order 2026-09-22 "dyn model 用 beta=1e-5
#     重新训"; the 09-21 threading s233 same-data comparison measured
#     |dKL/da|@0.25sigma 0.41 at beta 3e-5 vs 7.52 at 1e-5);
#   * ATY arm: per-round R-ANCHORED eta re-calibration (user-specified
#     2026-09-22, same method as COFFEE-MG-p4; scripts/coffee/cfk_rcalib.py)
#     injected after the walk-back VIB ckpt resolution and BEFORE GEXTRA:
#     (1) frozen core batch B=128 states; (2) R_mean at the PREVIOUS round's
#     dose via the real guided denoise loop; (3) eta_i = eta_{i-1}*0.01/R_mean;
#     (4) re-measure R_mean at eta_i, success iff 0.009 <= R_mean <= 0.011,
#     else repeat (2)(3), at most 2 repeats. kappa is CARRIED OVER unchanged
#     (kappa_0 = 2.5). eta_{i-1} for round 1 = ETA0 (1.0); later rounds read
#     $DATA_ROOT/$TASK/rollout/ATY-exp$((NUM-1))/calib_rcalib_r*.json.
#     Calib json + stdout land at $RDIR level (calib_rcalib_r$NUM.json /
#     calib.stdout: survive the phase-A restart rm of $RDIR/log). Calib
#     failure = round FATAL.
#   * DRY_RUN skips the calib call (planned dose = ETA0 / ATT_CAP).
# Everything else inherited VERBATIM from round_threading.sh (2026-09-16):
# THREE arms (DP: guide off / ATY: --guide atypical raw dose+kappa /
# ORBIT: --guide orbit same raw dose + lam.5/delta.25/sigma.05 x0.5^(r-1)/
# anneal2/fb-soft, NO dimless), ETRIES=5 (pass@5), SHARD_P=8/arm x25env,
# MODE=full|eval-only|eval-pk, two-phase sharded rescue, TSEED determinism,
# DP 300ep b256 retrain (BASE DP stays b64) / dyn 100ep SOE budget,
# walk-back, anti-deadlock retrain, heartbeat reporter, wandb backfill,
# flush-every.
#
...
# guide=off.
#
# Usage:  round_thm2.sh <threading> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
# Env:    GPU=<id> TSEED=<int> DATA_ROOT=<abs dir> (required)
#         WPROJ=<wandb project> ATT_CAP=2.5 ATY_SCALE=1.0 (fallbacks only)
#         ETA0=1.0 (rcalib eta anchor for round 1; kappa carried over)
#         BETA=1.0e-5 (VIB beta for every dyn training on this chain)
...
TASK=${1:?usage: round_thm2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_thm2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_thm2.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
WPROJ=${WPROJ:-THREADING-MG-p2-s${TSEED}}
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_thm2.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_thm2.sh $TASK BASE 0)"; exit 1; }
