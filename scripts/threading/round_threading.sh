#!/bin/bash
# round_threading.sh (2026-09-16) -- THREADING-MG-p1 round driver (MimicGen
# threading_d0 campaign). COPY of scripts/transport/round_tp.sh with these
# deltas ONLY:
#   * task = threading (MimicGen, env_name Threading_D0, horizon 400;
#     configs/vib_threading_exp1.yaml + configs/eval_threading_entropy.yaml +
#     configs/base_dp_threading_image.yaml, all new 2026-09-16);
#   * PY = /root/workspace/baojiachun/.venv_mg/bin/python (independent venv,
#     copy of the production .venv + mimicgen 1.0.1 -- user order 2026-09-16;
#     robomimic's env_robosuite imports mimicgen so Threading_D0 is
#     registered for EVERY env creation on this path);
#   * round0 OFFICIAL = the MimicGen threading_d0 pool (1000 mg-generated
#     demos, obs pre-rendered 84x84, states+actions OSC_POSE). The split's
#     20-demo core has NO abs_actions out of split_core.py -- the conversion
#     to 7-dim absolute actions is done ONCE per seed by
#     scripts/threading/thread_prep_data.sh BEFORE round0 (robomimic
#     dataset_conversion replays states in Threading_D0), so round0 finds
#     $CORE already converted and skips the split (same "core exists -- skip"
#     branch as tp15's copied-base flow);
#   * WPROJ default THREADING-MG-p1-s$TSEED.
# Everything else inherited verbatim from round_tp.sh: THREE arms (DP: guide
# off / ATY: --guide atypical raw dose+kappa2.5 / ORBIT: --guide orbit same
# raw dose + lam.5/delta.25/sigma.05 x0.5^(r-1)/anneal2/fb-soft, NO dimless),
# ETRIES=5 (pass@5), SHARD_P=8/arm x25env, MODE=full|eval-only|eval-pk,
# two-phase sharded rescue, TSEED determinism, DP 300ep b256 retrain (BASE
# DP stays b64) / dyn 100ep SOE budget, walk-back, anti-deadlock retrain,
# heartbeat reporter, wandb backfill, flush-every.
#
# Dose knobs are DEFAULTS ONLY -- thread_start_arms.sh pins the final
# post-grid values explicitly per arm (user flow: round0 -> grid on s233 ->
# arms start).
#
# Usage:  round_threading.sh <threading> <BASE|ATY|ORBIT|DP> <num> [full|eval-only|eval-pk]
...
TASK=${1:?usage: round_threading.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
A=${2:?usage: round_threading.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
NUM=${3:?usage: round_threading.sh <task> <BASE|ATY|ORBIT|DP> <num> [mode]}
case "$TASK" in
  can)      TASKUP=CAN ;;
...
ATY_SCALE=${ATY_SCALE:-0.5}     # atypical/orbit RAW dose (th95 default; threading dose set by the s233 grid; NO dimless)
...
WPROJ=${WPROJ:-THREADING-MG-p1-s${TSEED}}
...
[ -n "$(newest_ckpt "$TDP/DP-base")" ] || { echo "no DP-base ckpt (run: round_threading.sh $TASK BASE 0)"; exit 1; }
[ -n "$(newest_vib "$TDYN/dyn-base")" ] || [ "$A" = DP ] \
  || { echo "no dyn-base ckpt (run: round_threading.sh $TASK BASE 0)"; exit 1; }
