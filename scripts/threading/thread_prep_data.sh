#!/bin/bash
# thread_prep_data.sh -- per-seed core20 materialization for THREADING-MG-p1
# (2026-09-16). MimicGen threading_d0 pool (1000 mg-generated demos, obs
# PRE-RENDERED 84x84 agentview_image + robot0_eye_in_hand_image HWC uint8,
# states (T,45), actions (T,7) OSC_POSE, env_name Threading_D0) ->
...
#      replaying states in Threading_D0 (no rendering; no EGL needed)
#   3) rollout/threading_core.hdf5 in the campaign root + structure verify
...
# usage: bash scripts/threading/thread_prep_data.sh <SEED>
set -euo pipefail
SEED=${1:?usage: thread_prep_data.sh <SEED>}
ROOT=/root/workspace/baojiachun/scout
PY=/root/workspace/baojiachun/.venv_mg/bin/python
POOL=$ROOT/data/robomimic/threading/mg/threading_d0.hdf5
DST=$ROOT/data/2026_9_16_threading_p1/THREADING-MG-p1-s$SEED/threading
CORE=$DST/rollout/threading_core.hdf5
STAGE=$DST/rollout/.stage_core_noabs_s$SEED.hdf5

[ -f "$POOL" ] || { echo "[prep] FATAL: pool missing: $POOL"; exit 1; }
[ -f "$CORE" ] && { echo "[prep] core exists -- skip ($CORE)"; exit 0; }

# CPFS disk-write probe (df untrustworthy, 2026-09-10 lesson)
dd if=/dev/zero of=$ROOT/data/.th_prep_probe bs=1M count=512 oflag=direct 2>/dev/null \
  || dd if=/dev/zero of=$ROOT/data/.th_prep_probe bs=1M count=512
rm -f $ROOT/data/.th_prep_probe
...
echo "[prep] s$SEED step2: abs_actions conversion (state replay in Threading_D0)"
...
assert env_args["env_name"] == "Threading_D0", env_args["env_name"]
f.close()
