#!/bin/bash
# can824_walkcloud_probe.sh -- WalkCloud vs SCOUT(atypical) pass@5 A/B on
...
#     ONCE by step 0 below. Expect 43 failed / SR .57 (the drift campaign's
#     re-derivation; NOT the gae probe's 38 -- env batch size shifts the
#     eval sampling stream, NAMES.md CAN-8-24 records both).
...
#   * full 43-scene set in one round (~40 min at softmin cost = single
#     backward, walkcloud_plan §5 speed estimate); N=0 = full set, the
#     OFF/N window machinery is kept only as a fallback.
...
#
# usage: ROUND=1 [OFF=0] [N=0] [WC_TAU_MODE=adapt] [WC_TAU=0.5]
...
#        [ARMS="aty wc"] [ROOT=data/walkcloud_probe_can824]
# ARMS/ROOT exist for the 2026-09-10 k/gst sweep follow-ups: run the wc arm
# alone under a per-config ROOT (kappa/gst recorded in the ledger params key).
...
cd "$SCOUT_DIR" || { echo "[wc-probe] FATAL: scout dir $SCOUT_DIR missing"; exit 1; }
...
ROOT=${ROOT:-data/walkcloud_probe_can824}
CORE=$ROOT/can_core.hdf5
FAILED=$ROOT/failed_dp.json
T=$ROOT/r${ROUND}
[ -f "$DP" ] && [ -f "$VIB" ] && [ -f "$OFFICIAL" ] \
  || { echo "[wc-probe] FATAL: campaign ckpt / official dataset missing"; exit 1; }
mkdir -p "$ROOT" "$T/log"

# ---- step 0a: rebuild the seeded 20/200 core (deterministic, TSEED 233) -- #
if [ ! -f "$CORE" ]; then
  echo "[wc-probe] rebuilding core: split_core 20/200 seed 233"
  $PY scripts/analysis/split_core.py "$OFFICIAL" "$CORE" 20 233 \
    || { echo "[wc-probe] FATAL: split_core failed"; exit 1; }
fi
...
if [ ! -f "$FAILED" ]; then
  echo "[wc-probe] deriving failed set: DP-base eval seed42 x100 (GPU$GPU_WC)"
  ...
  rc=$?
  [ $rc -eq 0 ] && [ -f "$FAILED" ] \
    || { echo "[wc-probe] FATAL: failed-set derivation rc=$rc"; exit 1; }
fi
$PY - "$FAILED" <<'PYEOF'
import json, sys
spec = json.load(open(sys.argv[1]))
n = len(spec["failed_init_indices"])
print(f"[wc-probe] failed set: {n} inits, recorded SR "
      f"{spec.get('baseline_solved')}/100 "
      f"(drift campaign reference: 43 failed / SR .57; gae probe saw 38)")
PYEOF
[ "${PREP_ONLY:-0}" = "1" ] && { echo "[wc-probe] PREP_ONLY done"; exit 0; }
...
print(f"[wc-probe] window [{'0' if n <= 0 else str(off)}:{'all' if n <= 0 else str(off+n)}] = {idx}")
PYEOF
WIN=$T/win.json
...
run_arm() { # name gpu guide extra...
  ...
  local extra=("$@")
  echo "[wc-probe] arm=$name gpu=$gpu guide=$guide P=$P extra=${extra[*]:-} (backstop ${BACKSTOP}s)"
  local t0=$(date +%s)
  if [ "${DRY_RUN:-0}" = "1" ]; then
  ...
  local t1=$(date +%s)
  echo "$rc" > "$T/$name.rc"
  echo "[wc-probe] arm=$name rc=$rc wall=$(( (t1-t0)/60 ))m$(( (t1-t0)%60 ))s"
}

echo "[wc-probe] ROUND=$ROUND OFF=$OFF N=$N P=$P TRIES=$TRIES aty(scale/cap/gst)=$ATY_SCALE/$CAP/$GST wc(tau_mode/tau/hist_max)=$WC_TAU_MODE/$WC_TAU/$WC_HIST_MAX gpus aty/wc=$GPU_ATY/$GPU_WC"
...
wait
[ "${DRY_RUN:-0}" = "1" ] && { echo "[wc-probe] DRY_RUN done"; exit 0; }
...
if w_r is not None and a_r is None:
    print(f"[wc-probe] wc-only readout (no aty arm this run): "
    ...
    print(f"[wc-probe] VERDICT: {verdict} (rescued@5 wc={w_r:g} vs aty={a_r:g})")
    if w_r <= 14 and not partial:
        print("[wc-probe] PRE-REGISTERED FALSIFICATION LINE: wc<=14 -- if "
              "mean_S telemetry is normal, the i-axis falsification "
              "EXTENDS to kernel aggregation (walkcloud_plan §5): STOP.")
else:
    print("[wc-probe] VERDICT: INCOMPLETE (no rescued readout)")
...
if not new:
    ...
    for r in last.values():
        try:
            ...
            pass
    print("[wc-probe] CUMULATIVE (last row per round/arm/params): " + " ".join(
...
for arm in $ARMS; do
  if [ -f "$T/$arm/all.hdf5" ]; then
    $PY scripts/viz/vis_validate.py "$T/$arm/all.hdf5" \
      ...
      && echo "[wc-probe] $arm render: $(tail -1 $T/$arm.vischeck.txt)" \
      || echo "[wc-probe] $arm render: CHECK FAILED (see $T/$arm.vischeck.txt)"
  fi
done
echo "[wc-probe] ROUND $ROUND DONE $(date '+%F %T')"
