#!/bin/bash
# sq826_walkcloud_probe.sh -- WalkCloud vs SCOUT(atypical) pass@5 A/B on
...
#   * WINDOWED (square horizon 500 does not fit the full 63 in 45 min at
#     walkcloud speed): default N=32; cover with ROUND=1 OFF=0 N=32 +
#     ROUND=2 OFF=32 N=31 (ledger CUMULATES). N=0 = full set fallback.
...
#
# usage: ROUND=1 [OFF=0] [N=32] [WC_TAU_MODE=adapt] [WC_TAU=0.5]
...
[ -f "$DP" ] && [ -f "$VIB" ] && [ -f "$OFFICIAL" ] \
  || { echo "[sq-wc-probe] FATAL: campaign ckpt / official dataset missing"; exit 1; }
mkdir -p "$ROOT" "$T/log"

# ---- step 0a: rebuild the seeded 20/200 core (deterministic, TSEED 233) -- #
if [ ! -f "$CORE" ]; then
  echo "[sq-wc-probe] rebuilding core: split_core 20/200 seed 233 (square)"
  $PY scripts/analysis/split_core.py "$OFFICIAL" "$CORE" 20 233 \
    || { echo "[sq-wc-probe] FATAL: split_core failed"; exit 1; }
fi
...
if [ ! -f "$FAILED" ]; then
  echo "[sq-wc-probe] deriving failed set: DP-base eval seed42 x100 (GPU$GPU_WC)"
  ...
  rc=$?
  [ $rc -eq 0 ] && [ -f "$FAILED" ] \
    || { echo "[sq-wc-probe] FATAL: failed-set derivation rc=$rc"; exit 1; }
fi
$PY - "$FAILED" <<'PYEOF'
import json, sys
spec = json.load(open(sys.argv[1]))
n = len(spec["failed_init_indices"])
print(f"[sq-wc-probe] failed set: {n} inits, recorded SR "
      f"{spec.get('baseline_solved')}/100 "
      f"(drift campaign reference: 63 failed / SR .37)")
PYEOF
[ "${PREP_ONLY:-0}" = "1" ] && { echo "[sq-wc-probe] PREP_ONLY done"; exit 0; }
...
print(f"[sq-wc-probe] window [{'0' if n <= 0 else str(off)}:{'all' if n <= 0 else str(off+n)}] = {idx}")
PYEOF
WIN=$T/win.json
...
run_arm() { # name gpu guide extra...
  ...
  local extra=("$@")
  echo "[sq-wc-probe] arm=$name gpu=$gpu guide=$guide P=$P extra=${extra[*]:-} (backstop ${BACKSTOP}s)"
  local t0=$(date +%s)
  if [ "${DRY_RUN:-0}" = "1" ]; then
  ...
  local t1=$(date +%s)
  echo "$rc" > "$T/$name.rc"
  echo "[sq-wc-probe] arm=$name rc=$rc wall=$(( (t1-t0)/60 ))m$(( (t1-t0)%60 ))s"
}

echo "[sq-wc-probe] ROUND=$ROUND OFF=$OFF N=$N P=$P TRIES=$TRIES aty(scale/cap/gst)=$ATY_SCALE/$CAP/$GST wc(tau_mode/tau/hist_max)=$WC_TAU_MODE/$WC_TAU/$WC_HIST_MAX gpus aty/wc=$GPU_ATY/$GPU_WC"
...
if w_r is not None and a_r is not None:
    ...
    print(f"[sq-wc-probe] VERDICT: {verdict} (rescued@5 wc={w_r:g} vs aty={a_r:g})")
    if w_r <= 14 and not partial:
        print("[sq-wc-probe] PRE-REGISTERED FALSIFICATION LINE: wc<=14 -- if "
              "mean_S telemetry is normal, the i-axis falsification "
              "EXTENDS to kernel aggregation (walkcloud_plan §5): STOP.")
else:
    print("[sq-wc-probe] VERDICT: INCOMPLETE (no rescued readout)")
...
for arm in aty wc; do
  if [ -f "$T/$arm/all.hdf5" ]; then
    $PY scripts/viz/vis_validate.py "$T/$arm/all.hdf5" \
      ...
      && echo "[sq-wc-probe] $arm render: $(tail -1 $T/$arm.vischeck.txt)" \
      || echo "[sq-wc-probe] $arm render: CHECK FAILED (see $T/$arm.vischeck.txt)"
  fi
done
echo "[sq-wc-probe] ROUND $ROUND DONE $(date '+%F %T')"
