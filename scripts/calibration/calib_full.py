"""Visual-slice (1024 = both views) gate-threshold calibration from the
cached r2 bank + e500t154 visited states. -> quantiles for a possible
'visual' arm. ~4 min (bank cached)."""
import sys
...
VIS = slice(None)   # full s_bar (1088) = both views + proprio
STRIDE = 5
...
def main():
    ...
    bank = torch.load(CACHE, map_location=dev)[:, VIS]
    ...
    print(f"n_states={len(d)}  visual-slice NN distance to r2 bank:")
