#!/bin/bash
# SQUARE s23333 orbit-native 6-round chain (user order 2026-09-02; replica of
# s233 sq_orb_chain.sh with seed swapped, GPU6).
...
# 20260829-025739 + core_rebuild/square_core_s23333.hdf5), linked/copied into
...
# Usage: GPU=<id> bash /tmp/sq_orb_chain_23333.sh
set -uo pipefail
...
DATA_ROOT=$ROOT/data/2026_9_1_orbchain/ORBIT-s23333
T=$DATA_ROOT/square
WPROJ=SQUARE-9-1-orbit-s23333

mkdir -p "$T/rollout" "$T/train/DP/DP-base/checkpoints" "$T/train/dyn/dyn-base"
[ -f "$T/rollout/square_core.hdf5" ] || cp "$R26/core_rebuild/square_core_s23333.hdf5" "$T/rollout/square_core.hdf5"
ln -sf "$R26/SQUARE-entropy-s23333/square/train/DP/DP-base/checkpoints/599.ckpt" \
      "$T/train/DP/DP-base/checkpoints/599.ckpt"
ln -sfn "$R26/SQUARE-entropy-s23333/square/train/dyn/dyn-base/20260829-025739" \
       "$T/train/dyn/dyn-base/20260829-025739"
