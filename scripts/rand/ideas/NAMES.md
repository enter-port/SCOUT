...
| **GAElike** | — | entropy cost 的 GAE 式历史化:cost_t = −Σ_{k=0}^{t−1} γ^k·min(KL(q_t‖q_k), κ)/Σγ^k,q_k=锚点以来第 k 个 guided 去噪步的意图后验(γ→0 精确退化为 entropy cost;归一化默认开=剂量连续)(2026-09-08,branch GAElike-dev) | `--guide gaelike --gae-gamma/--gae-norm`,gae_costs.py |
| **WalkCloud** | — | 去噪步 x̂₀ 累积云 drifting 排斥:同 chunk 去噪 walk 每步把注入前的 x̂₀ 后验 push 进云,候选对云做 soft-min 核排斥 S=−τ̃·logΣ_j exp(−KL(q_t‖H_j)/τ̃),cost=−min(S,κ);t=1 空云不引导、t=2 云={x̂₀₁}=锚⇒逐位 atypical,差异从第 3 步起;纯 cost 替换、零引擎改动(2026-09-10,branch drift-dev,idea/walkcloud_plan.md;与 GAElike 同 i 轴但聚合=核 soft-min 而非 γ^k 加权和——i 轴在加法聚合下证伪,核聚合未测) | `--guide walkcloud --cloud-tau/--cloud-tau-mode/--cloud-hist-max`,walk_cloud_costs.py |
...
| **GAE 折扣 γ(gae-gamma)** | GAElike 术语:历史第 k 项的权重 γ^k(锚点 k=0 权重 1,越新越小)——保证历史和几何有界(≤κ/(1−γ));γ=0 只剩锚点项=entropy cost 逐位等价 |
| **WalkCloud(去噪步 x̂₀ 云)** | 同一 chunk 去噪 walk 内、每个 guided 步注入前的一步 x̂₀ 估计经冻结 VIB 编码的后验,按步累积成参照云;每 chunk 经 select_z 重置;t=1 的 push 即锚(与 select_z 看到同一注入前 x̂₀,编码器确定性) |
| **soft-min 核(drifting 排斥核)** | S=−τ̃·logΣ_j exp(−KL_j/τ̃):τ̃→0 退化 hard-min(最近参照主导),核权重 softmax(−KL/τ̃) 和恒 1=位移有界;与 GAElike 的 γ^k 加权和(每个参照都施压、合力无归一)相对——WalkCloud 与 GAElike 的全部区别就在这个聚合形式 |
| **τ̃ adapt(核温自适应)** | 逐行 τ̃=clamp(0.3·池内 KL 极差, 0.02, 0.5)(cloudrep iter-2 规则移植);绝对 τ 在压缩 KL 尺度(如 square)上等效大 τ→权重平坦→核力被平均稀释 |
