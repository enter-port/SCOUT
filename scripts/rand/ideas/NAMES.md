...
| **cloudrep** | 方案一(drift) | 云参照排斥:soft-min_τ KL 到 {当前 chunk 锚} ∪ {本场景已重试的 chunk-0 锚云},梯度=drifting 归一化核(softmax 权重);空云(retry 0)逐位=entropy cost(τ 为 2 的幂;一般 τ 注入逐位等价、值差 ulp 级)| `--guide cloudrep`,cloudrep_costs.py |
...
| **云(cloud)/ 软最小温度 τ(cloudrep-tau)** | cloudrep 术语:场景锚云=该场景历次重试 chunk-0 捕获的 (μ⁰,σ⁰²) 参照集(同初态,可直接比较);S=−τ·log Σ exp(−KL_j/τ) 为池内 KL 的软最小,τ 小→逃离最近已试目标,τ 大→对云平均;重试 0 池={锚}单元素,S≡KL(逐位=entropy cost,τ 取 2 的幂) |
| **cloudrep-ort(正交云排斥)** | cloudrep 的 agg=ort 形态:云软最小梯度投影到锚点逃逸法平面后以 λ 加进爬升(g = mask·(g_a + λ·g_c⊥)),平行分量(锚聚簇时的纯剂量冗余)=伤害源被删,切向分量=真重定向被保留;κ 预算只算锚(与 aty 停止语义逐字同);遥测 cos/φ/lam_ratio 为可证伪读数 |
