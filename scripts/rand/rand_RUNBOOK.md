...
## 设计约束(用户令 2026-08-27 晚,最高优先)
1. **KL divergence 的 cost 形式基本不动**:随机性注入在它周围/内部
   (参考度量、目标方向、剂量、时域门),不是把 cost 换成别的东西
   (动作空间线性倾斜/替换目标函数都不行)。
2. **随机性不必是 SOE 的形式**(不要为"像 SOE"而设计)。
3. **dyn model 结构绝不能动**(冻结 VIB 编码器/解码器,不重训)。
4. **优化目标 = 同一场景的重试生成相对 diverse 的 action**
   (重试间多样性),同时救回/pass@10 不低于方案三。

## 治理规则(用户令 2026-08-28,orchestrator 执行)
1. **三振出局**:同一思路连续修改三轮都不对 → 不是参数熟悉度问题,是思路本身不可行 → 停止该线,派一个反思 subagent:把失败机制总结为可复用的经验(写进 soe_scripts/rand_ideas/LESSONS.md),并按准入标准重选方向。
2. **新思路准入标准**:简洁——没有必要的部分就不要加(部件数最小化);最好有理论依据(能说清为什么该随机化在该空间该形式下应该 work,而不是"试试看")。
